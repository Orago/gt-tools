...
import { C_RTFILE, eCompressionType } from './config.js';
import { RTFileHeader, RTPackHeader, RTTEXHeader, RTTEXMipHeader } from './structures.js';
import sharp from 'sharp';
const { C_COMPRESSION_ZLIB } = eCompressionType;
function hashString(buffer) {
    let hash = 0x55555555;
    if (buffer)
...
export async function writeImageToRttex(ITRF, path) {
...
export async function writeRttexToImage(RFTI, path, flipVertical = true) {
