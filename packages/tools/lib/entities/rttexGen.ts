...
export class RttexEntityGenerator extends EngineObject {
		...
		if (clickToDownload) {
			engine.cursor.events.on("click", () => {
				...
				for (const ent of this.rttexList) {
					...
					if (
						Collision.rectContains(box, {
							x: engine.cursor.pos.x,
							y: engine.cursor.pos.y,
							width: 1,
							height: 1,
						})
					) {
			...
			engine.cursor.events.on("context", () => {
				let y = 0;
				let i = 0;
		...
		this.events.on("render", () => {
			...
			for (const { sprite } of this.rttexList) {
				engine.brush.chainable
					...
					.size(
						sprite.width * engine.zoom,
						sprite.height * engine.zoom
					)
					.image(sprite);
	...
	async handleFiles(files: File[]) {
		for (const file of files) {
			...
			if (result != null)
				this.rttexList.push(new RttexSprite(file, result));
		}
	}
