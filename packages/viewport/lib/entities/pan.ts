...
function getTotalDistance(points: Point[]) {
	let totalDistance = 0;
...
class PCHandler {
	...
	panTick() {
		...
		if (PARENT.modes.panning && PARENT.pan.active)
			this.panMove();
	}
...
export class PanningPlugin extends EngineObject {
	...
	private handleWheel(event: WheelEvent) {
		...
		const zoom = (1.1 * this.options.pcFactor) ** (event.deltaY < 0 ? 1 : -1);
	...
	public factorZoom(zoom: number, pos: Point) {
